import { Bell, Clock, Check, AlertCircle, Info, type LucideIcon } from 'lucide-react'
...
import { useInstructorNotifications } from '../hooks/useInstructorSupplements'
import type { DashboardRole, InstructorNotificationItem } from '../services/dashboard.api'

const InstructorNotificationsPage: React.FC = () => {
  const { user, logout } = useAuth()
  const navigate = useNavigate()
  const { data: notifications = [], isLoading, isError, error, refetch } = useInstructorNotifications()
  ...
  const typeConfig: Record<InstructorNotificationItem['type'], { icon: LucideIcon; color: string; bg: string }> = {
    info: { icon: Info, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-400/10' },
    warning: { icon: AlertCircle, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-400/10' },
    success: { icon: Check, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-400/10' },
    error: { icon: AlertCircle, color: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-400/10' },
  }
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 overflow-hidden">
          <div className="p-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
            ...
            <button type="button" className="text-xs text-blue-600 dark:text-blue-400 opacity-60 cursor-default font-medium" disabled>Synchronise automatiquement</button>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-white/5">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="text-xs text-slate-400 dark:text-white/30 flex-shrink-0">{new Date(notif.date).toLocaleDateString('fr-FR')}</span>
                    </div>
            ...
            {notifications.length === 0 && (
              <div className="p-10 text-center text-sm text-slate-500 dark:text-white/40">
